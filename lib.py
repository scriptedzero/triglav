...
class SwitchNode:
	...
	def __repr__(self):
		return (
			f'(switch {self.condition!r}\n '
			+ (' ' * INDENTATION)
			+ ('\n ' + ' ' * INDENTATION).join(
				f'case {case_cond!r}\n ' + (' ' * INDENTATION * 2) + f'{case_body!r}'
				for case_cond, case_body in list(self.cases)
			)
			+ '\n '
			+ (' ' * INDENTATION)
			+ 'else\n'
...
class NamespaceNode:
	...
	def __repr__(self):
		return f"""(namespace {self.name.value if self.name is not None else ''}\n{
			self.body!r
		}\nend)"""
...
class Parser:
	...
	def dot(self):
		...
		while self.current_tok.type == TokenType.DOT:
			...
			node = DotGetNode(
				node, self.current_tok, node.pos_start, self.current_tok.pos_end
			)
			self.advance(res)
		...
		if self.current_tok.type == TokenType.EQ and isinstance(node, DotGetNode):
			...
			node = DotSetNode(
				node.noun, node.verb, value, node.pos_start, self.current_tok.pos_end
			)
	...
	def atom(self):
		...
		elif tok.matches(TokenType.KEYWORD, 'fun') or tok.matches(
	...
	def for_expr(self):
		...
		if self.current_tok.type != TokenType.EQ and not self.current_tok.matches(
		...
		if self.current_tok.type == TokenType.NEWLINE:
			...
			if is_for_in:
				return res.success(
					ForInNode(var_name, iterable_node, body, pos_start, pos_end, True)
				)
			return res.success(
		...
		if is_for_in:
			return res.success(
				ForInNode(var_name, iterable_node, body, pos_start, pos_end, False)
			)
		return res.success(
	...
	def struct_def(self):
		...
		self.advance(res)
		return res.success(
			StructNode(name=name, fields=fields, pos_start=pos_start, pos_end=pos_end)
		)
...
class Value:
	...
	def illegal_operation(self, *others):
		...
		return RTError(
			self.pos_start, others[-1].pos_end, 'Illegal operation', self.context
		)
...
class BuiltInFunction(BaseFunction):
	...
	def execute_open(self, exec_ctx):
		...
		try:
			...
			if isinstance(err, TypeError):
				...
				return res.failure(
					RTError(fake_pos, fake_pos, f"Cannot find file '{fn}'", exec_ctx)
				)
			else:
				return res.failure(
					RTError(fake_pos, fake_pos, f'{err.args[-1]}', exec_ctx)
				)
	...
	def execute_read(self, exec_ctx):
		...
		try:
			...
			return res.failure(
				RTError(fake_pos, fake_pos, f'Invalid file descriptor: {fd}', exec_ctx)
			)

		return res.success(
			String(result).set_pos(fake_pos, fake_pos).set_context(exec_ctx)
		)
	...
	def execute_write(self, exec_ctx):
		...
		return res.success(
			Number(num).set_pos(fake_pos, fake_pos).set_context(exec_ctx)
		)
	...
	def execute_close(self, exec_ctx):
		...
		if fd < 3:
			return res.failure(
				RTError(fake_pos, fake_pos, f'Cannot close {std_desc[fd]}', exec_ctx)
			)
...
class Dict(Value):
	...
	def gen(self):
		...
		for key in self.values.keys():
			key_as_value = (
				String(key).set_pos(fake_pos, fake_pos).set_context(self.context)
			)
...
class StructInstance(Value):
	...
	def copy(self):
		return (
			StructInstance(self.struct_name, self.fields)
			.set_pos(self.pos_start, self.pos_end)
			.set_context(self.context)
		)
...
class Interpreter:
	...
	def visit_StructCreationNode(self, node, ctx):
		...
		return res.success(
			StructInstance(node.name, {field: Number.null for field in struct})
			.set_pos(node.pos_start, node.pos_end)
			.set_context(ctx)
		)
...
def run(fn, text, context=None, entry_pos=None):
	...
	if context_was_none and ret:
		if not isinstance(ret, Number):
			return None, RTError(
				ret.pos_start, ret.pos_end, 'Exit code must be Number', context
			)
		exit(ret.value)
